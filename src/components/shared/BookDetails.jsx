...
// Removed: import { SUBJECTS } from "../constants/subjects"; // No longer needed, now using dynamic subjects
import { formatPrice } from "../utils/formatPrice";
import { UploadFile } from "@/api/integrations"; // Added UploadFile
import { useSubjects } from "../hooks/useSubjects"; // Added useSubjects hook
...
export default function BookDetails({
  ...
  // BEGIN: cover-image upload helpers
  async function handleCoverUpload(e) {
    const file = e?.target?.files?.[0];
    if (!file) return;

    try {
      // Optional: simple inline "busy" mark by writing a temporary flag into form data (no new props)
      setEditFormData(prev => ({ ...prev, __coverUploading: true }));

      const { file_url } = await UploadFile({ file }); // must resolve to a public URL string
      setEditFormData(prev => ({ ...prev, cover_image_url: file_url, __coverUploading: false }));
    } catch (err) {
      setEditFormData(prev => ({ ...prev, __coverUploading: false }));
      // The `notify` function is not available in this component's scope.
      console.error("Cover upload failed:", err);
    } finally {
      // reset file input so re-selecting the same file will retrigger onChange
      if (e?.target) e.target.value = "";
    }
  }

  function handleRemoveCover() {
    setEditFormData(prev => ({ ...prev, cover_image_url: "" }));
  }
  // END: cover-image upload helpers
      ...
      <div className="flex-1">
        {isEditMode ?
          <div className="space-y-4">
            ...
            {/* Cover Image URL Input (replaced with device upload; layout/classes preserved) */}
            <div>
              <Label htmlFor="cover_image_url">תמונת שער</Label>
              <Input
                id="cover_image_url"
                type="file" // Changed to file type
                accept="image/*" // Added accept attribute
                onChange={handleCoverUpload} // Changed handler
                className="mt-1"
                disabled={editFormData?.__coverUploading} // Added disabled state
              />
              {/* Minimal inline controls WITHOUT changing surrounding layout */}
              {editFormData?.cover_image_url && !editFormData?.__coverUploading ? (
                <button
                  type="button"
                  onClick={handleRemoveCover}
                  className="text-xs text-red-600 underline mt-2 mr-2" // Added mr-2 for spacing if text is LTR
                >
                  הסר תמונה קיימת
                </button>
              ) : null}
              {editFormData?.__coverUploading ? (
                <span className="mr-2 text-xs text-gray-500">מעלה…</span> // Added mr-2 for spacing if text is LTR
              ) : null}
            </div>
            ...
            <div className="space-y-1">
              ...
              {isEditMode ? (
                ...
                >
                  <SelectTrigger id="subject" className="text-right w-full mt-1">
                    <SelectValue placeholder={subjectsLoading ? "טוען נושאים..." : "בחירת נושא"} />
                  </SelectTrigger>
                  <SelectContent>
                    {subjects.map((subject) => (
                      <SelectItem key={subject.id} value={subject.name} className="text-right">
                        {subject.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <p className="font-semibold">{displayBook?.subject || 'לא צוין'}</p>
              )}
            </div>
